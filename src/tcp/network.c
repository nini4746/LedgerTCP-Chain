				 ...
				 const void *data, size_t size)
{
	...
	if (!sim || !data || size == 0)
		return -1;
	if (from < 0 || from >= sim->node_count)
		return -1;
	if (to < 0 || to >= sim->node_count)
		return -1;
	if (!sim->nodes[from].is_active)
		return -1;
	if (!sim->nodes[to].is_active)
		return -1;
	packet = packet_create(from, to, data, size, sim->current_time);
	if (!packet)
		return -1;
					...
					void *buffer, size_t buffer_size)
{
	...
	if (!sim || !buffer || buffer_size == 0)
		return -1;
	if (node_id < 0 || node_id >= sim->node_count)
		return -1;
	if (!sim->nodes[node_id].is_active)
		return -1;
	packet = queue_dequeue(sim->nodes[node_id].inbox);
	if (!packet)
		return 0;
	sent = packet_serialize(packet, buffer, buffer_size);
						  ...
						  packet_t *packet)
{
	...
	{
		dup = packet_copy(packet);
		if (dup)
			queue_enqueue(node->pending, dup);
	}
