...
ledger_t *ledger_clone(const ledger_t *ledger) {
    ...
    ledger_t *clone = ledger_create();
    if (!clone) return NULL;

    memcpy(clone->accounts, ledger->accounts,
           sizeof(account_t) * ledger->account_count);
    clone->account_count = ledger->account_count;

    memcpy(clone->history->tx_ids, ledger->history->tx_ids,
           sizeof(tx_id_t) * ledger->history->tx_count);
...
void ledger_clear(ledger_t *ledger) {
    ...
    memset(ledger->history->tx_ids, 0,
           sizeof(ledger->history->tx_ids));
}

bool ledger_validate(const ledger_t *ledger) {
    if (!ledger) return false;
    if (!ledger->history) return false;
