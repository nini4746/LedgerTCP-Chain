...
{
	chain_candidate_t *fork;

	if (!chain || !state)
	{
		return NULL;
	}
	fork = malloc(sizeof(chain_candidate_t));
	if (!fork)
	{
		return NULL;
	}
...
int fork_add_candidate(consensus_node_t *node, blockchain_t *chain, ledger_t *state)
{
	...
	if (node->fork_count >= MAX_CHAINS)
	{
		return -1;
	}
	fork = fork_create(chain, state);
	if (!fork)
	{
		return -1;
	}
...
blockchain_t *fork_get_longest(consensus_node_t *node)
{
	...
	chain_candidate_t *current;

	if (!node)
	{
		return NULL;
	}
	longest = node->main_chain;
	current = node->fork_chains;
	while (current)
	{
